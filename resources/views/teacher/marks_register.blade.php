                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="form-group col-md-3">
                            ...
                            <a href="{{ url('teacher/marks_register') }}" style="margin-top: 30px"
                            ...
                            <tr>
                                ...
                                <td>
                                    <div style="margin-bottom: 10px">
                                        ...
                                        <input type="text" name="mark[{{ $i }}][class_work]" id="class_work_{{ $student->id }}{{ $subject->subject_id }}"
                                            style="width: 100%;" class="form-control" value="{{ !empty($getMark->class_work) ? $getMark->class_work : '' }}"
                                            placeholder="Enter Marks Obtained">
                                    </div>
                                    <div style="margin-bottom: 10px">
                                        Home Work Marks
                                        <input type="text" name="mark[{{ $i }}][home_work]" id="home_work_{{ $student->id }}{{ $subject->subject_id }}"
                                            style="width: 100%;" class="form-control" value="{{ !empty($getMark->home_work) ? $getMark->home_work : '' }}"
                                            placeholder="Enter Marks Obtained">
                                    </div>
                                    <div style="margin-bottom: 10px">
                                        Tests Marks
                                        <input type="text" name="mark[{{ $i }}][test_work]" id="test_work_{{ $student->id }}{{ $subject->subject_id }}"
                                            style="width: 100%;" class="form-control" value="{{ !empty($getMark->test_work) ? $getMark->test_work : '' }}"
                                            placeholder="Enter Marks Obtained">
                                    </div>
                                    <div style="margin-bottom: 10px">
                                        Exam Marks
                                        <input type="text" name="mark[{{ $i }}][exam]" id="exam_{{ $student->id }}{{ $subject->subject_id }}"
                                            style="width: 100%;" class="form-control" value="{{ !empty($getMark->exam) ? $getMark->exam : '' }}"
                                            placeholder="Enter Marks Obtained">
                                    </div>

                                    <div style="margin-bottom: 10px">
                                        Teacher's Comments
                                        <textarea name="mark[{{ $i }}][teacher_comments]" id="teacher_comments_{{ $student->id }}{{ $subject->subject_id }}"
                                            style="width: 100%;" class="form-control" rows="3"
                                            placeholder="Teacher's Comments">{{ !empty($getMark->teacher_comments) ? $getMark->teacher_comments : '' }}</textarea>
                                    </div>

                                    <div style="margin-bottom: 10px">
                                        <button type="button" class="btn btn-primary SaveSingleSubject" id="{{ $student->id }}" data-val='{{ $subject->subject_id }}'
                                            data-schedule="{{ $subject->id }}" data-exam="{{ Request::get('exam_id') }}" data-class="{{ Request::get('class_id') }}">SAVE
                                        </button>
                                    </div>
                                    @if(!empty($getMark))
                                    <div style="margin-bottom: 10px">
                                ...
                                <td style="min-width: 300px">
                                    ...
                                    <b>Total Subjects Mark </b>{{ $totalFullMarks }} <br />
                                    <b>Total Passing Mark </b>{{ $totalPassingMarks }} <br />
                                    @php
